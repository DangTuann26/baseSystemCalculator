    ...
    public class BaseSystemCalculator_Layout extends JFrame {
      ...
      public BaseSystemCalculator_Layout(String title) {
            // Gọi phương thức setTitle() để đặt tiêu đề cho cửa sổ
            this.setTitle(title);

            // Thêm các điều khiển và sự kiện (đây là các phương thức bạn tự định nghĩa)
            addControls();
            addEvents();

            try {
                // Tạo URL cho hình ảnh
                URL imageUrl = new URL("https://i.imgur.com/aCz364W.png");
                ...
                // Đặt hình ảnh làm icon cho cửa sổ
                setIconImage(image);
            } catch (MalformedURLException e) {
                e.printStackTrace();  // In lỗi nếu URL không hợp lệ
            } catch (IOException e) {
                e.printStackTrace();  // In lỗi nếu có vấn đề khi tải hình ảnh
            }
        }
